...
describe('WaitForHelper', () => {
  ...
  it('returns when a synchronous dialog blocks the action response', async () => {
    await withMcpContext(async (_response, context) => {
      const mcpPage = context.getSelectedMcpPage();
      try {
        const result = await Promise.race([
          mcpPage.waitForEventsAfterAction(() =>
            mcpPage.pptrPage.evaluate(() => alert('synchronous dialog')),
          ),
          new Promise<'stalled'>(resolve =>
            setTimeout(() => resolve('stalled'), 5000),
          ),
        ]);
        assert.notStrictEqual(result, 'stalled');

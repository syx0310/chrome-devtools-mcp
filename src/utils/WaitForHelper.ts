...
export class WaitForHelper {
  ...
  async waitForEventsAfterAction(
    ...
    const unhandledDialog = Promise.withResolvers<void>();
    const dialogHandler = (
    ...
    try {
      // A synchronous dialog can hold the action's CDP response until the
      // user handles it. Release the tool so handle_dialog can run.
      await Promise.race([action(), unhandledDialog.promise]);
    } catch (error) {
